...
def phoneGW():
    User_phone = input(
        f"\n {Wh}Enter phone number target {Gr}Ex [+97798xxxxxxxx] {Wh}: {Gr}"
    )
    ...
    # FIX: correct geocoder
    location = geocoder.description_for_number(parsed_number, "en")

    # FIX: prevent Kathmandu fallback
    if not location or location.lower() in ["", "nepal"]:
        location = "Nepal (exact city not available)"
    ...
    formatted_mobile = phonenumbers.format_number_for_mobile_dialing(
        parsed_number,
        default_region,
    ...
    print(f"{Wh} Timezone             :{Gr} {', '.join(timezones)}")
    print(f"{Wh} Operator             :{Gr} {operator}")
...
def TrackLu():
    ...
    for site in social_media:
        ...
        try:
            ...
            # safer check (not perfect but better)
            if response.status_code == 200 and username.lower() in response.text.lower():
                print(f"{Gr}[ FOUND ] {site['name']} : {url}")
            else:
                print(f"{Ye}[ NOT SURE ] {site['name']} : {url}")

        except:
            print(f"{Re}[ ERROR ] {site['name']} : Request failed")
@is_option
def showIP():
